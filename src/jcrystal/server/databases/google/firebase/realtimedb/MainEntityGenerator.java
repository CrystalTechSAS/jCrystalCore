...
public class MainEntityGenerator extends AbsEntityGenerator{
	...
	private class EntityGenerator extends InternalEntityGenerator{
		...
		protected void generateConstructors() {
			...
			$("public " + entidad.getTipo() + "(Entity." + entidad.mainDBType.getDBName() + " sourceEntity)", ()->{
				if(entidad.padre != null)
					$("super(sourceEntity);");
				else {
					$("this.rawEntity = new java.util.TreeMap<>(sourceEntity.getRawEntity());");
				}
			});
			$("public " + entidad.getTipo() + "(com.google.firebase.database.DataSnapshot sourceData)", ()->{
		...
		protected void generateSaveMethods() {
			if(entidad.entidad.internal())
				return;
			$("public void realtimePut(String path) throws InterruptedException, java.util.concurrent.ExecutionException",()->{
				$("jcrystal.context.CrystalContext $ctx = jcrystal.context.CrystalContext.get();");
				$("$ctx."+entidad.mainDBType.getDBName()+"().databaseReference(path).setValueAsync(getRawEntity(), null).get();");
			});
			$("public void realtimePatch(String path) throws InterruptedException, java.util.concurrent.ExecutionException",()->{
				$("jcrystal.context.CrystalContext $ctx = jcrystal.context.CrystalContext.get();");
				$("$ctx."+entidad.mainDBType.getDBName()+"().databaseReference(path).updateChildrenAsync(getRawEntity()).get();");
			});
			$("public static void realtimeDelete(String path) throws InterruptedException, java.util.concurrent.ExecutionException",()->{
				$("jcrystal.context.CrystalContext $ctx = jcrystal.context.CrystalContext.get();");
				$("$ctx."+entidad.mainDBType.getDBName()+"().databaseReference(path).setValueAsync(null, null).get();");
			});
			$("public static " + entidad.clase.getSimpleName() + " realtimeGet(String path) throws InterruptedException",()->{
				$("jcrystal.context.CrystalContext $ctx = jcrystal.context.CrystalContext.get();");
				$("jcrystal.context.Utils"+StringUtils.capitalize(entidad.mainDBType.getDBName())+".SingleValueEventListener $listener = new jcrystal.context.Utils"+StringUtils.capitalize(entidad.mainDBType.getDBName())+".SingleValueEventListener();");
				$("synchronized ($listener)",()->{
					$("$ctx.realtime().databaseReference(path).addListenerForSingleValueEvent($listener);");
					$("do",()->{
						$("$listener.wait();");
					},"while(!$listener.complete);");
				});
				$("return new "+entidad.clase.getSimpleName()+"($listener.snapshot);");
			});
		}
		@Override
		...
		@Override public void generateExtras() {
			...
			$("public static java.util.List<" + entidad.getTipo()+"> convertRawList(java.util.List<com.google.firebase.database.DataSnapshot> rawData)", ()->{
				$if("rawData == null", "return null;");
				$else(()->{
					$("java.util.ArrayList<"+entidad.getTipo()+"> ret = new java.util.ArrayList<>();");
					$("for(com.google.firebase.database.DataSnapshot data : rawData)",()->{
